...
if (!self.__WB_pmw) {
	...
	function recordSignUp() {
		_gaq.push(["_trackPageview", "/pageTracker/ajax/signUpComplete"])
	}
	...
	function showSignup(e, c) {
		...
		_gaq.push(["_trackPageview", "/pageTracker/ajax/overlay/signup/true"]);
		_kmq.push(["record", "Opened signup overlay"])
	}
		...
		jQuery.ajax({
			...
			success: function (e) {
				if (e.error) {
					...
					if (b == "movie") {
						displayFeedback("0" + GT.gettext("Watermark assigned to movie"))
					} else {
						displayFeedback("0" + GT.gettext("Watermark assigned to user"))
					}
				}
			},
			error: function () {
			...
			gaLogTx: {
				createCCPartLogger: function (c) {
					...
					this.submit = function () {
						...
						_gaq.push(["_addTrans", g, "GoAnimate Store", "0", "0.00", "0.00", "", "", ""]);
						for (itmId in e) {
							_gaq.push(["_addItem", g, itmId, itmId, (h > 0 ? ("template #" + h) : "template"), "0", e[itmId]])
						}
				...
				logCCPartsNormal: function (h, e, g) {
					...
					var k = "AS" + h;
					_gaq.push(["_addTrans", k, "GoAnimate Store", "0", "0.00", "0.00", "", "", ""]);
					for (c in j) {
						_gaq.push(["_addItem", k, c, c, (g > 0 ? ("template #" + g) : "custom"), "0", j[c]])
					}
					_gaq.push(["_trackTrans"])
				},
				logActionpack: function (e, c, g) {
					return;
					var f = "AP" + e + "_" + c;
					_gaq.push(["_addTrans", f, "GoAnimate Store", "0", "0.00", "0.00", "", "", ""]);
					_gaq.push(["_addItem", f, c, g, "actionpack", "0", "1"]);
					_gaq.push(["_trackTrans"])
				}
			},
